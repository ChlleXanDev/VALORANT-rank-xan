...
# os.system('cls')
os.system(f"title VALORANT rank xan v{version}")
...
try:
    ...
    try:
        if len(sys.argv) > 1 and sys.argv[1] == "--config":
            ...
            run_app = inquirer.confirm(
                message="Do you want to run vRX now?", default=True
            ).execute()
            if run_app:
    ...
    log(f"VALORANT rank xan v{version}")
    ...
    print(color("\nVisit https://vrx.netlify.app/matchLoadouts to view full player inventories\n", fore=(255, 253, 205)))
    chatlog(color("\nVisit https://vrx.netlify.app/matchLoadouts to view full player inventories\n", fore=(255, 253, 205)))
    ...
    while True:
        ...
        if True:
            ...
            if title is not None:
                ...
                table.set_caption(f"VALORANT rank xan v{version}")
                table.display()
                firstPrint = False

                # print(f"VALORANT rank xan v{version}")
                # chatlog(f"VALORANT rank xan v{version}")
                                        #                 {
